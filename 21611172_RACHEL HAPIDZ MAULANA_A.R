...
ui <- navbarPage(
  ...
  # Descriptive Statistics (moved to the first position)
  tabPanel("Multiple Linear Regression",
           fluidPage(
             titlePanel(title = div("Multiple Linear Regression", style = "color: #3E3F3A; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             mainPanel(
  ...
  tabPanel("Correlation Plot",
           fluidPage(
             titlePanel(title = div("Correlation Plot", style = "color: #3E3F3A; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             mainPanel(
               style = "margin: 0 auto; text-align: center; margin-left: 300px;",
  ...
  tabPanel("Variable Relationship",
           fluidPage(
             titlePanel(title = div("Variable Relationship Analysis Dashboard", style = "color: #3E3F3A; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             sidebarLayout(
               sidebarPanel(
  ...
  tabPanel("Prediction Dashboard",
           fluidPage(
             titlePanel(title = div("Private Hospital Personnel Needs Prediction Dashboard and Monthly Working Hours", style = "color: #3E3F3A; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             sidebarLayout(
               sidebarPanel(
                 numericInput("x1_input", "x1 Value:", value = 0),
                 numericInput("x2_input", "x2 Value:", value = 6000),
                 numericInput("x3_input", "x3 Value:", value = 0),
                 numericInput("x4_input", "x4 Value:", value = 20),
...
server <- function(input, output) {
  ...
  output$barplot_output <- renderPlotly({
    plots <- lapply(2:7, function(i) {
      plot_ly(datafix, x = ~data[, i], type = "histogram", color = I("#3E3F3A")) %>%
        layout(showlegend = FALSE, title = colnames(datafix)[i], yaxis = list(title = "Frequency"))
    })
  ...
  observeEvent(input$plot, {
    ...
    output$scatter_boxplot <- renderPlot({
      ...
      scatter_plot <- ggplot(data, aes_string(x = variable1, y = "y")) +
        geom_point(color = "#3E3F3A", size = 3) +
